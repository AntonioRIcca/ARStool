...
from sympy import exp, Symbol, oo, nsimplify
...
# from __shared__.plugins.fiabilipy import Component
from Functionalities.Adequacy.__plugins__.fiabilipy import Component
__all__ = ['System']

#ALLSUBSETS = lambda n: (chain(*[combinations(range(n), ni)
                        #for ni in range(n+1)]))

ALLSUBSETS = lambda n: (chain.from_iterable([combinations(range(n), ni)
                        for ni in range(n+1)]))
...
class System(object):
    ...
    def _probabilitiescomputation(self, t, method):
        ...
        for S in ALLSUBSETS(len(paths)):
            ...
            r = exp(-float('%.3e' % float(str(reduce(lambda x, y: x*getattr(y, method)(t), comps, 1)).
                                     replace('exp(-', '').replace('*t)', '')))*t)

            R += -r if len(S) % 2 == 0 else r
        return R

    def _probabilitiescomputation_mod(self, t, time, method):
        """ Given a system and a `method` (either availability or
            maintainability or reliability), this method evaluates the asking
            value by exploring the graph at time `t`.
        """
        ...
        for S in ALLSUBSETS(len(paths)):
            ...
            comps = set([c for i in S for c in paths[i][1:-1]])
            r = exp(-float(str(reduce(lambda x, y: x*getattr(y, method)(t), comps, 1)).replace('exp(-', '').
                           replace('*t)', ''))*time)
            R += -r if len(S) % 2 == 0 else r
        return R
    ...
    def reliability(self, t):
        ...
        # if isinstance(t, Symbol):
        #     # print('4e')
        #     return nsimplify(formula)
        # else:
        #     # print('4f')
        #     return formula.subs(self._t, t).evalf()

        return self._probabilitiescomputation_mod(self._t, t, 'reliability')
    ...
    def faulttreeanalysis(self, output=None, order=2):
        r""" Build the fault tree analysis of the system
            ...
            PDF (or write) the content of the dot file needed to draw the
